...
class CompactBilinearPooling(nn.Module):
    ...
    Args:
        ...
        rand_s_2: (Optional) an 1D numpy array of 1 and -1, having the same shape
                  ...
                  None.
        output_dim: 紧凑双线性池化的输出维度。

        sum_pool: （可选）如果为True，则沿着高度和宽度维度对输出进行求和，并返回形状为[batch_size, output_dim]的输出。
          否则返回形状为[batch_size, height, width, output_dim]的输出。
          默认值为True。

        rand_h_1: （可选）一个包含在区间 `[0, output_dim)` 中的一维numpy数组。如果为None，则从`seed_h_1`自动生成。

        rand_s_1: （可选）一个形状与`rand_h_1`相同的由1和-1组成的一维numpy数组。如果为None，则从`seed_s_1`自动生成。

        rand_h_2: （可选）一个包含在区间 `[0, output_dim)` 中的一维numpy数组。如果为None，则从`seed_h_2`自动生成。

        rand_s_2: （可选）一个形状与`rand_h_2`相同的由1和-1组成的一维numpy数组。如果为None，则从`seed_s_2`自动生成。
        cuda: (Optional) if True, use cuda. Default: True.
    """

    def __init__(self, input_dim1=640, input_dim2=640, output_dim=800,
...
#     layer = CompactBilinearPooling(640, 640, 8000)
#     layer.cuda()
#     layer.train()
